 ...
 */
function standardizeColor(name: string): any {
 ...
 * @return {number[]} HSV values.
 */
function cmykToHsv(c: number, m: number, y: number, k: number): number[] {
  c /= 100;
  m /= 100;
  y /= 100;
  k /= 100;
 ...
 * @return {number[]} HSV values.
 */
function hslToHsv(h: number, s: number, l: number): number[] {
  s /= 100;
  l /= 100;
  s *= l < 0.5 ? l : 1 - l;
 ...
 * @return {object}
 */
export function parseToHSVA(str: string): object {
